...
if (Yii::$app->controller->action->id === 'pdf') { 
/**
...
if (Yii::$app->controller->action->id === 'relog') { 
/**
 * Do not use this code in your template. Remove it. 
 * Instead, use the code  $this->layout = '//main-login'; in your controller.
 */
...
js;
    ...
    <div id="float" class="fixed">
        <div class="col-lg-4" id="chatingclose" style="display: block;">
                <a href="#" type="button" id="openchat" class="float">
    ...
    <script type="text/javascript">
        $('input[name="r"]').each(function() {
          $( this ).after( '<input type="hidden" name="FlagMobile" value="true">' );
          $( this ).after( '<input type="hidden" name="IdUser" value="'+<?= $_GET['IdUser'] ?>+'">' );
        });

        $('a').each(function() {
          href_lama = $( this ).attr('href');
          // href_lama = href_lama.replace("#", "");
          $( this ).attr('href',href_lama+'&FlagMobile=true&IdUser='+<?= $_GET['IdUser'] ?>);
        });
    </script>
    <?php
      }
    ...
    function listUser(){
       ...
       $.ajax({
            ...
            success: function (response) {
                  ...
                  $('#lists').empty();
                  $.each(obj, function( index, value ) {
                    if (value.status_read>0) {
                      $('<li class="lichat"><a href="#" onclick="saveRoom(\''+value.id+'_'+id+'\',\''+value.username+'\')" ><span class="notification">'+value.username+'<span class=\"badges\" style="font-size: 18px;"><b>*</b></span></span></a></li>').appendTo($('#lists'));
                    }else{
                      $('<li class="lichat"><a href="#" onclick="saveRoom(\''+value.id+'_'+id+'\',\''+value.username+'\')" ><span class="notification">'+value.username+'</span></a></li>').appendTo($('#lists'));
                    }
                  });
            },
            error: function(jqXHR, textStatus, errorThrown) {
