...
<div class="billing-final-form">
	...
	<div class="row">
        ...
        <!-- <div class="col-md-12"> -->
            ...
            <div class="col-md-5">
            <?php 
                $realpath=$model->tbs_path_resume;
                $petjah = explode('/', $model->tbs_path_resume);
                $model->tbs_path_resume = $petjah[count($petjah)-1];
            ?>
            <?= $form->field($model, 'tbs_path_resume')->textInput(['disabled' => true])->label('File Permintaan Tindak') ?>
            </div>
             <div class="col-md-1" style="padding-top: 25px">
                <?= Html::a('View File',Url::to(['persetujuan/viewpdf','filename'=>$realpath]), ['class' => 'btn btn-success', 'target' => '_blank'
                // 'onclick'=>'theFunction("'.Yii::$app->params['downloadPermintaan'].$model->tpt_path_permintaan_tindak.'"); return false;',
        ...
        <div class="col-md-8">
            <?= $form->field($model, 'tbs_flag_status')->dropDownList(
                ...
                ['prompt'=>'Pilih Keputusan...'])->label('Keputusan (ACC tarif PKS Telkom, Alkes non Medis Dan Suplemen ditagihkan ditempat)');
            ?>
        </div>
        <div class="col-md-8">
...
<script type="text/javascript">
	...
	function daftarData(val) {
		$.ajax({
            url: '<?= Url::to(["billing-final/daftar"]) ?>'+'&nikes='+val+'&td_id='+<?= $model['tbs_td_id'] ?>, 
            success: function(result){
                ...
                $('#billingfinal-peserta').val(obj.td_nama_kel);
                $('#billingfinal-pegawai').val(obj.td_tp_nama_kk);
                $('#billingfinal-hakkelas').val(obj.td_hak_kelas);
                $('#billingfinal-jenis_peserta').val(obj.jenis_peserta);
            }
        });
	}
