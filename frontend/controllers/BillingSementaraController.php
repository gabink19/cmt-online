...
use frontend\models\BillingSementaraSearch;
...
class BillingSementaraController extends MainController 
{
    ...
    public function uploadFile($tempName,$fileName,$nikes='') 
    {
        ...
        if (!move_uploaded_file($tempName, Yii::$app->params['pathBillingSementara'] .date('Y').'/'.date('m').'/'. $fileName)) {
            return '';
        }
        return Yii::$app->params['pathBillingSementara'] .date('Y').'/'.date('m').'/'.$fileName;
    }
    public function actionBilling($value='')
    {
        ...
        return $this->render('billingsementara', [
   ...
   public function actionDaftar($nikes,$newrecord,$td_id='')
    {
        ...
        if (!$result && $nikes!='') {
            ...
            if ($td_id!='') {
                    $data = 'SELECT count(*) FROM tbl_billing_final WHERE tbs_tp_nikes="'.$nikes.'" and tbs_td_id="'.$td_id.'"  and flag_deleted=0 order by tbs_id LIMIT 1';
            }
        ...
        if ($model->load(Yii::$app->request->post())){
            ...
            if ($result>0) {
                if (strtolower($fileExtensions) =='pdf' && $size <= 3) {
                    ...
                    if ($model->save()) {
                            $query = 'UPDATE tbl_daftar SET td_flag_status="4" WHERE td_id = "'.$model->tbs_td_id.'" AND td_tp_nikes="'.$model->tbs_tp_nikes.'" AND flag_deleted="0"' ;
                            $result = Yii::$app->db->createCommand($query)->execute();
                            ...
                            $activity = "[CREATE BILING SEMENTARA] ".$data;
                            $this->historyUser($activity,$user,$ip);
                            // $modelNotif = new Notifikasi();
                            // $modelNotif->tn_kepada = 6;
                            // $modelNotif->tn_tanggal = date('Y-m-d H:i:s');
                            // $modelNotif->tn_judul = 'Billing Sementara';
                            // $modelNotif->tn_teks = 'Billing Sementara pada nikes '.$model->tbs_tp_nikes.'.';
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($_FILES['BillingSementara']['name']['tbs_path_billing']!='') {
                $name = $_FILES['BillingSementara']['name']['tbs_path_billing'];
                $tmp_name = $_FILES['BillingSementara']['tmp_name']['tbs_path_billing'];
                $size = number_format($_FILES['BillingSementara']['size']['tbs_path_billing'] / 1048576, 2);
                $fileExplode = explode('.', $name);
    ...
    public function actionDelete($id)
    {
        ...
        if($model->delete()) {
            ...
            $activity = "[DELETE BILLING SEMENTARA] ".$data;
            $this->historyUser($activity,$user,$ip);
