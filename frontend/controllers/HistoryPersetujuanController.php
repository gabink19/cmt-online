...
// die();
    ...
    public function actionDownloadpdf($id,$uniq_code)
    {   
        $filename = Yii::$app->params['temp_pdf'].$id.$uniq_code.'_historypersetujuan.pdf';
        $file_exist = file_exists($filename);
        if ($file_exist) {
            unlink($filename);
        }
        shell_exec("/usr/local/bin/wkhtmltopdf --page-size 'A4' --orientation 'landscape' --page-width '1024' --enable-javascript --encoding 'UTF-8'  'https://www.cmt-online.site/index.php?r=history-persetujuan%2Fpdf&id=".$id."&uniq_code=".$uniq_code."' '".$filename."'");

        return $this->redirect(['/persetujuan/download', 'filename' => $filename]);
    }


    public function actionPdf($id,$uniq_code)
    {
        $this->layout = false;
